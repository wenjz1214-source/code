...
def assign_instances_to_previous_tracks(
  ...
  """
  # When there are no detections, TensorFlow may hand us rank-1 empty tensors.
  # Normalizing the shape upfront keeps slicing and tf.while_loop logic stable.
  prev_centers = tf.reshape(prev_centers, [-1, 5])
  current_centers = tf.reshape(current_centers, [-1, 5])
