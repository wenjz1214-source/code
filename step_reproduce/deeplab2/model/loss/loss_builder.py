...
class DeepLabFamilyLoss(tf.keras.layers.Layer):
  ...
  def call(self, y_true: Dict[Text, tf.Tensor],
    ...
    for loss_name, func_and_weight in (
      ...
      # Do not blanket zero non-finite values here: it makes training look
      # "all zeros" while weights may already be NaN and no gradients flow.
      # Use model-side fixes (e.g. motion center heatmap) + trainer grad hygiene.
