...
class MotionDeepLab(tf.keras.Model):
  ...
  def __init__(self,
    ...
    # MOTChallenge-STEP is trained with batch size 1 in this workspace. The
    # BatchNorm moving statistics become too poor for inference, while per-batch
    # statistics still produce meaningful semantic and center predictions.
    self._use_batch_stats_for_eval = (
        dataset_descriptor.dataset_name == 'motchallenge_step' and
        not config.trainer_options.solver_options.use_sync_batchnorm)
  ...
  def call(self, input_tensor: tf.Tensor, training=False) -> Dict[Text, Any]:
    ...
    network_training = training
    if not training and self._use_batch_stats_for_eval:
      network_training = True
    ...
    pred = self._decoder(
        self._encoder(input_tensor, training=network_training),
    ...
    # Bound unstable heads in both train and eval. This prevents non-finite
    # logits/offsets from poisoning the post-processing and evaluation losses.
    if common.PRED_SEMANTIC_LOGITS_KEY in result_dict:
      lg = result_dict[common.PRED_SEMANTIC_LOGITS_KEY]
      lg = tf.where(
          tf.math.is_finite(lg),
          tf.clip_by_value(lg, -80.0, 80.0),
          tf.zeros_like(lg))
      result_dict[common.PRED_SEMANTIC_LOGITS_KEY] = lg
    for _off_key in (common.PRED_OFFSET_MAP_KEY,
                     common.PRED_FRAME_OFFSET_MAP_KEY):
      if _off_key in result_dict:
        ot = result_dict[_off_key]
        result_dict[_off_key] = tf.where(
            tf.math.is_finite(ot),
            tf.clip_by_value(ot, -512.0, 512.0),
            tf.zeros_like(ot))
    ...
    if common.PRED_CENTER_HEATMAP_KEY in result_dict:
      ch = tf.squeeze(result_dict[common.PRED_CENTER_HEATMAP_KEY], axis=3)
      ch = tf.where(
          tf.math.is_finite(ch),
          tf.clip_by_value(ch, -20.0, 20.0),
          tf.zeros_like(ch))
      result_dict[common.PRED_CENTER_HEATMAP_KEY] = ch
    return result_dict
