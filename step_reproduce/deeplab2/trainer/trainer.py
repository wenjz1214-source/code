...
def _zero_non_finite_gradients(
    gradients: list) -> list:
  """Replaces non-finite gradient components with zeros (keeps apply_gradients safe)."""
  out = []
  for g in gradients:
    if g is None:
      out.append(None)
...
class Trainer(orbit.StandardTrainer):
  ...
  def __init__(self, config: config_pb2.ExperimentOptions,
    ...
    # MirroredStrategy + regular BatchNorm + tf.while_loop (steps_per_loop>1)
    # can hit aggregate_ops "lists of tensors with different lengths" on TF 2.6.
    # Python inner loop avoids that (slightly slower but stable).
    super(Trainer, self).__init__(
        train_dataset,
        StandardTrainerOptions(use_tf_while_loop=False))
  ...
  def _train_step(self, inputs: Dict[Text, Any]):
    ...
    with tf.GradientTape() as tape:
      ...
      for name, loss in loss_dict.items():
        average_loss_dict[name] = tf.reduce_mean(loss)

      total_loss = average_loss_dict[common.TOTAL_LOSS]
      scaled_loss = total_loss / self.strategy.num_replicas_in_sync
      # One bad step must not backprop NaN/Inf into weights.
      scaled_loss = tf.where(
          tf.math.is_finite(scaled_loss),
          scaled_loss,
          tf.zeros_like(scaled_loss))

    training_vars = self._model.trainable_variables
    gradients = tape.gradient(scaled_loss, training_vars)
    gradients = _zero_non_finite_gradients(gradients)
